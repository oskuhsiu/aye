...
#[test]
fn opening_history_starts_at_newest_window_even_from_old_closed_context() {
    let mut app = many();
    app.snapshot
        .state
        .tasks
    ...
    assert_eq!(app.selected_id, Some(id(0)));
    key(&mut app, KeyCode::Esc);
    assert_eq!(app.selected_id, Some(id(124)));
}
#[test]
fn recent_region_preserves_graph_and_live_history_selection() {
    let mut app = many();
    ...
    key(&mut app, KeyCode::Enter);
    assert!(frame(&mut app, 55, 25).contains("CLOSED ROW 000"));
    key(&mut app, KeyCode::Esc);
    key(&mut app, KeyCode::Char('h'));
    for _ in 0..70 {
        key(&mut app, KeyCode::Down);
    }
    assert_eq!(app.selected_id, Some(id(70)));
    ...
    state.tasks.insert(newest.id.clone(), newest);
    ...
    assert_eq!(app.selected_id, Some(id(70)));
    let text = frame(&mut app, 180, 30);
    assert!(text.contains("CLOSED ROW 070"));
    assert!(app.history.as_ref().unwrap().rendered_rows <= 26);
    key(&mut app, KeyCode::Esc);
    assert!(app.history.is_none());
    frame(&mut app, 180, 30);
